...
- (void)viewDidLoad {
    ...
    for (int i=0; i<3; i++) {
        if (month-i <= 0) {
            [self.currentMonths addObject:@{@"title": [NSString stringWithFormat:@"%ld-%ld", year-1, month-i+12], @"year" : @(year-1), @"month": @(month-i+12)}];
        }
        else {
            [self.currentMonths addObject:@{@"title": [NSString stringWithFormat:@"%ld-%ld", year, month-i], @"year" : @(year), @"month": @(month-i)}];
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == self.selectList) {
        ...
        cell.array = [self.dataArray objectAtIndex:0];
        return cell;
    }
    return nil;
}
