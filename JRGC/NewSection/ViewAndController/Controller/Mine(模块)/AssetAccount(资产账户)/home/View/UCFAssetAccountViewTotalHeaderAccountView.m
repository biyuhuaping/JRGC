...
@property (nonatomic, strong) MyRelativeLayout *bLayout;// 黄金
...
- (void)reloadAccountLayout:(UCFAccountCenterAssetsOverViewModel *)model
{
    if (nil == model || !model.ret) {
        ...
        [model.data.assetList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            
            UCFAccountCenterAssetsOverViewAssetlist *ass = obj;
            
            MyRelativeLayout *bkLayout = [MyRelativeLayout new];
            bkLayout.backgroundColor = [UIColor clearColor];
            bkLayout.padding = UIEdgeInsetsMake(0, 0, 0, 0);
            bkLayout.myWidth = LayoutWidth;
            bkLayout.heightSize.equalTo(self.rootLayout.heightSize);
            bkLayout.topPos.equalTo(self.rootLayout.topPos);
            [self.rootLayout addSubview:bkLayout];
            
            UIView *roundView= [UIView new];
            roundView.myLeft = 0;
            roundView.myCenterY = 0;
            roundView.myWidth = 7;
            roundView.myHeight = 7;
            roundView.viewLayoutCompleteBlock = ^(MyBaseLayout *layout, UIView *sbv)
            {
                //设置圆角的半径
                sbv.layer.cornerRadius = 3.5;
                //切割超出圆角范围的子视图
            ...
            [bkLayout addSubview:roundView];
            
            NZLabel *titleLabel = [NZLabel new];
            titleLabel.myCenterY = 0;
            titleLabel.myLeft = LabelLeft;
            titleLabel.textAlignment = NSTextAlignmentLeft;
            titleLabel.font = [Color gc_Font:14.0];
            titleLabel.textColor = [Color color:PGColorOptionTitleBlack];
            [bkLayout addSubview:titleLabel];
            
            if ([ass.type isEqualToString:@"P2P"]){
                titleLabel.text = @"微金账户";
                [titleLabel sizeToFit];
                roundView.backgroundColor = PNBlue;
            }
            else if ([ass.type isEqualToString:@"ZX"]) {
                
                titleLabel.text = @"尊享账户";
                [titleLabel sizeToFit];
                roundView.backgroundColor = PNOrange;
            }
            else if ([ass.type isEqualToString:@"GOLD"]) {
                titleLabel.text = @"黄金账户";
                [titleLabel sizeToFit];
                roundView.backgroundColor = PNYellow;
            }
            
            if (model.data.assetList.count == 2) {
                if (idx == 0) {
                    bkLayout.myCenterX = - 70;
