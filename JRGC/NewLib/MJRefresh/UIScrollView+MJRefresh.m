...
- (void)addMyGifHeaderWithRefreshingTarget:(id)target refreshingAction:(SEL)action{
    NSArray *refreshingImages = [self praseGIFDataToImageArray:[NSData dataWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"gif@3x.gif" withExtension:nil]]];
    [self addGifHeaderWithRefreshingTarget:target refreshingAction:action];
    [self.gifHeader setImages:refreshingImages forState:MJRefreshHeaderStateIdle];
    ...
    [self.header setTitle:@"             松开刷新" forState:MJRefreshHeaderStatePulling];
    [self.header setTitle:@"                   正在加载中..." forState:MJRefreshHeaderStateRefreshing];
    
    // 设置颜色
    self.header.textColor = UIColorWithRGB(0x999999);
    self.footer.textColor = UIColorWithRGB(0x999999);
