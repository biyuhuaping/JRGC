...
#import "UCFMicroBankOpenAccountChangeBankCardInfoModel.h"
...
@interface UCFMicroBankDepositoryChangeBankCardViewController ()<UIScrollViewDelegate,UITextFieldDelegate,UCFHuiShangChooseBankViewControllerDelegate>
...
//    是否支持快捷支付
    self.isQuick = [data[@"isQuick"]boolValue];
...
- (void)changebankIdRequest
{
    UCFMicroBankOpenAccountChangeBankCardInfoAPI * request = [[UCFMicroBankOpenAccountChangeBankCardInfoAPI alloc] initWithBankCard:[self.bankNumView.contentField.text stringByReplacingOccurrencesOfString:@" " withString:@""] bankId:self.bankId validateCode:self.smsView.contentField.text openStatus:self.GetOpenAccountModel.data.openStatus AccoutType:self.accoutType];
    request.animatingView = self.view;
    //    request.tag =tag;
    [request startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        // 你可以直接在这里使用 self
        DDLogDebug(@"---------%@",request.responseJSONModel);
        UCFMicroBankOpenAccountChangeBankCardInfoModel *model = [request.responseJSONModel copy];
        if (model.ret == YES)
        {
            if(model.code == 10000) {
...
//
        ...
        {
            ShowCodeMessage(self.GetOpenAccountModel.code, self.GetOpenAccountModel.message);
        }
    } failure:^(__kindof YTKBaseRequest * _Nonnull request) {
        // 你可以直接在这里使用 self
...
- (void)statVerifyCodeRequest:(NSString *)isVms
{
    ...
    UCFMicroBankIdentifysendCodeInfoAPI * request = [[UCFMicroBankIdentifysendCodeInfoAPI alloc] initWithDestPhoneNo:self.GetOpenAccountModel.data.userInfo.phoneNum isVms:isVmsNew type:@"3" AccoutType:SelectAccoutTypeP2P];
    request.animatingView = self.view;
    //    request.tag =tag;
    [request startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        // 你可以直接在这里使用 self
