...
- (void)clickRightBtn
{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"1.返现券和返息券可在一笔投资中共用\n2.返现券可叠加使用\n3.返息券只能使用一张,不可叠加" delegate:self cancelButtonTitle:@"我知道了" otherButtonTitles: nil];
    [alert show];
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
//        }
        ...
        if (btn.selected) {
            ...
            [[NetworkModule sharedNetworkModule] postReq:strParameters tag:kSXtagHuoQuAllYOUHuiQuan owner:self];
        } else {
            self.couponSum = 0.0f;
...
- (void)reloadView
{
    ...
    [[NetworkModule sharedNetworkModule] postReq:strParameters tag:kSXTagCheckMyMoney owner:self];
}
...
//    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    if (_listType == 0) {
        ...
        [[NetworkModule sharedNetworkModule] postReq:strParameters tag:kSXtagSelectBeanRecord owner:self];
    } else if (_listType == 1) {
        beansPageNum = 1;
        ...
        [[NetworkModule sharedNetworkModule] postReq:strParameters tag:kSXtagSelectBeansInterest owner:self];
    }
