...
- (void)startCountDown {
    ...
    [self setTitleColor:[Color color:PGColorOptionCellContentBlue] forState:UIControlStateNormal];
...
- (void)refreshButton {
    ...
    if (_restCountDownNum == 0) {
        [self.timer invalidate];
        self.timer = nil;
        if (_startCountDownNum < 60) {
            _startCountDownNum = 60;
        }
        _isCountDown = NO;//倒计时结束,设为no
        _restCountDownNum = _startCountDownNum;
...
{
    return  _startCountDownNum;
}

- (NSUInteger)getRestDownNum
{
    return  _restCountDownNum;
}
