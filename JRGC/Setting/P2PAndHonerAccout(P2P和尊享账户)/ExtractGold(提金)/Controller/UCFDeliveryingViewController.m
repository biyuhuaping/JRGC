//
//  UCFDeliveryingViewController.m
//  JRGC
//
...
@interface UCFDeliveryingViewController () <UITableViewDataSource, UITableViewDelegate>
...
@implementation UCFDeliveryingViewController
...
- (void)viewDidLoad {
    ...
    // 添加传统的下拉刷新
    [self.tableview addMyGifHeaderWithRefreshingTarget:self refreshingAction:@selector(getExtractGoldListDeliveryingFromNet)];
    // 添加上拉加载更多
    [self.tableview addLegendFooterWithRefreshingBlock:^{
        [weakSelf getExtractGoldListDeliveryingFromNet];
    }];
...
- (void)getExtractGoldListDeliveryingFromNet
{
    ...
    NSString *userId = [UserInfoSingle sharedManager].userId;
    NSMutableDictionary *param = [NSMutableDictionary dictionaryWithObjectsAndKeys:userId, @"userId", pageNo, @"pageNo", @"3", @"status", @"20", @"pageSize", nil];
