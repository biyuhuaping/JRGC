...
- (void)segmentedValueChanged:(UISegmentedControl *)sender{
    ...
    if (sender.selectedSegmentIndex==2) {
        [self transitionFromViewController:self.currentController toViewController:weakSelf.myClaimCtrl duration:0 options:UIViewAnimationOptionLayoutSubviews animations:nil completion:^(BOOL finished) {
            if (finished) {
        ...
        [self transitionFromViewController:self.currentController toViewController:weakSelf.myInvest duration:0 options:UIViewAnimationOptionLayoutSubviews animations:nil completion:^(BOOL finished) {
            if (finished) {
