...
+ (MB_INSTANCETYPE)showHUDAddedToFullScreen {
    ...
    MBProgressHUD *hud = [[self alloc] initWithView:app.loadingBaseView];
    hud.mode = MBProgressHUDModeCustomView;
    UIImageView *view1 = [[[UIImageView alloc] initWithFrame:CGRectMake(0,0, 91, 95.5)] autorelease];
    view1.image = [UIImage sd_animatedGIFNameWithOneGif:@"loading"] ;
    ...
    [app.loadingBaseView addSubview:hud];
    [hud show:YES];
...
+ (void)displayHudError:(NSString *)errStr withShowTimes:(double)time
{
    if (![errStr isKindOfClass:[NSString class]]
        || errStr.length == 0) {
        return;
